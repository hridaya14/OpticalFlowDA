...
from core.memflow.core.utils import frame_utils
from core.memflow.core.utils.augmentor_video import FlowAugmentor, SparseFlowAugmentor
from core.memflow.core.utils.utils import forward_interpolate
import pickle
...
class CrossDomainFlow(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.forward_warp:
            ...
            return torch.stack(clean_imgs), torch.stack(foggy_imgs), torch.stack(flows), torch.stack(valids), torch.stack(forward_warped_flow)
...
class KITTI_Foggy(CrossDomainFlow):
    def __init__(self, aug_params=None, input_frames=5, forward_warp=False):
        super(KITTI_Foggy, self).__init__(aug_params=aug_params, input_frames=input_frames, sparse=True, forward_warp=forward_warp)
        ...
        for idx_list in range(200):
            for idx_image in range(0, input_frames - 1):
                # Build sequence of input_frames
                clean_seq = [(root + "KITTI-multiview/training/image_2/000{:03}_{:02}.png".format(idx_list, i - idx_image + 10)) for i in range(input_frames)]
                foggy_seq = [(root + "KITTI-multiview/training/image_2/000{:03}_{:02}.png".format(idx_list, i - idx_image + 10)) for i in range(input_frames)]

                self.image_list.append(clean_seq)
                self.foggy_image_list.append(foggy_seq)
                self.flow_list.append([root + "KITTI/training/flow_occ/000{:03}_10.png".format(idx_list)] * (input_frames - 1))
                self.has_gt_list.append([False] * idx_image + [True] + [False] * (input_frames - 2 - idx_image))

        print(self.image_list[0:3])
        print(self.foggy_image_list[0:3])
        print(self.flow_list[0:3])
        print(self.has_gt_list[0:3])
