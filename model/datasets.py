...
class CrossDomainFlow(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, input_frames=5,
                 forward_warp=False, subsample_groundtruth=False):
        self.augmentor = None
        self.sparse = sparse
        self.input_frames = input_frames
        self.forward_warp = forward_warp
    ...
    def __getitem__(self, index):
        ...
        index = index % len(self.clean_image_list)
        ...
        if not self.sparse:
            ...
            flows = []
            valids = []
            for i in range(len(self.has_gt_list[index])):
                if self.has_gt_list[index][i]:
                    flow, valid = frame_utils.readFlowKITTI(self.flow_list[index][i])
                    flows.append(flow)
                    valids.append(valid)
                else:
                    flow, valid = frame_utils.readFlowKITTI(self.flow_list[index][i])
        ...
        clean_imgs = [frame_utils.read_gen(path) for path in self.clean_image_list[index]]
        ...
        # Ensure 3-channel
        for domain_imgs in [clean_imgs, foggy_imgs]:
            if len(domain_imgs[0].shape) == 2:
                domain_imgs[:] = [np.tile(img[..., None], (1, 1, 3)) for img in domain_imgs]
            else:
                domain_imgs[:] = [img[..., :3] for img in domain_imgs]

        if self.augmentor is not None:
            if self.sparse:
                clean_imgs, flows, valids = self.augmentor(clean_imgs, flows, valids)
                foggy_imgs, _, _ = self.augmentor(foggy_imgs, flows, valids)  # Optional
            else:
                clean_imgs, flows = self.augmentor(clean_imgs, flows)
                foggy_imgs, _ = self.augmentor(foggy_imgs, flows)  # Optional
        ...
        if not self.forward_warp:
            return (torch.stack(clean_imgs), torch.stack(foggy_imgs),
                    torch.stack(flows), torch.stack(valids))
        else:
            new_size = (flows[0].shape[1] // 8, flows[0].shape[2] // 8)
            if not o_valids:
                downsampled_flow = [F.interpolate(flow.unsqueeze(0), size=new_size, mode='bilinear', align_corners=True).squeeze(0) / 8 for flow in flows[:-1]]
    ...
    def __rmul__(self, v):
        self.flow_list = v * self.flow_list
        self.clean_image_list = v * self.clean_image_list
...
class KITTI_Foggy(CrossDomainFlow):
    def __init__(self, aug_params=None, input_frames=5, forward_warp=False):
        super(KITTI_Foggy, self).__init__(
        ...
        root_clean = 'datasets/KITTI/KITTI-multiview/training/image_2/'
        root_foggy = 'datasets/KITTI/KITTI-multiview_foggy/training/image_2/'
        flow_root = 'datasets/KITTI/training/flow_occ/'
        ...
        for idx_list in range(200):
            for idx_image in range(0, input_frames - 1):
                # Collect clean and foggy frame paths for the current sequence
                clean_seq = [
                    f"{root_clean}000{idx_list:03}_{i - idx_image + 10:02}.png"
                    for i in range(input_frames)
                ]
                foggy_seq = [
                    f"{root_foggy}000{idx_list:03}_{i - idx_image + 10:02}.png"
                    for i in range(input_frames)
                ]
                self.clean_image_list.append(clean_seq)
                self.foggy_image_list.append(foggy_seq)

                # Repeat same GT flow for (input_frames - 1) steps
                self.flow_list.append(
                    [f"{flow_root}000{idx_list:03}_10.png"] * (input_frames - 1)
                )

                # Set has_gt_list so that only one flow per sequence has valid GT
                self.has_gt_list.append(
                    [False] * idx_image + [True] + [False] * (input_frames - 2 - idx_image)
                )

        print("Sample clean images:", self.clean_image_list[0])
        print("Sample foggy images:", self.foggy_image_list[0])
        print("Sample flow GTs:", self.flow_list[0])
        print("Sample has_gt:", self.has_gt_list[0])
